...
export const getActiveCoupons = async (storeName: string,status:string,page: number,itemsPerPage:number): Promise<Offer[]> => {
  try {
    ...
    const response = await fetch(`http://localhost:3000/api/public/offers?store_website=https://${storeName}&status=${status}&page=${page}&itemsPerPage=${itemsPerPage}`);
...
export const getAllActiveCouponsUptoPage = async (storeName: string,status:string,page: number,itemsPerPage:number,upto : boolean = true): Promise<Offer[]> => {
  try {
    ...
    const response = await fetch(`http://localhost:3000/api/public/offers?store_website=https://${storeName}&status=${status}&page=${page}&itemsPerPage=${itemsPerPage}&upto=${upto}`);
